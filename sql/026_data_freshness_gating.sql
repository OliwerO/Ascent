-- =============================================
-- 026: Data freshness gating for hard overrides
--
-- Resolves the policy conflict between CLAUDE.md ("Body Battery and Training
-- Readiness are context-only") and the operational code (TR<40 / BB<30 as
-- hard overrides). The resolution: keep these as safety guard rails but
-- ONLY when data is fresh (<12 hours old). Stale data triggered a false
-- rest override on 2026-04-09 — this migration prevents that class of bug.
--
-- Changes:
--   daily_coaching_context — adds data_age_hours column, gates hard_override
--                            on data freshness for device-only metrics
-- =============================================


-- Must drop because we're adding data_age_hours before the existing hard_override column
...
WITH
  ...
  -- Recovery data with freshness tracking
  recovery AS (
    SELECT
  ...
  -- NEW (migration 026): data freshness from daily_metrics.synced_at
  data_freshness AS (
    SELECT
      ROUND(
...
SELECT
  ...
  -- NEW (migration 026): data freshness
  df.data_age_hours,
  -- hard_override now gated on data freshness for device-only metrics
  -- Subjective wellness and multi-signal (HRV+sleep) are NOT gated because
  -- they don't depend on a single Garmin sync timestamp.
  CASE
