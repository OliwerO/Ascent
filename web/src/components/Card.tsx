import type { ReactNode } from 'react'

interface Props {
  title?: string
  subtitle?: string
  children: ReactNode
  className?: string
  glow?: 'green' | 'yellow' | 'red' | 'blue'
}

const glowStyles = {
  green: 'shadow-[0_0_24px_rgba(52,211,153,0.06)] border-accent-green/20',
  yellow: 'shadow-[0_0_24px_rgba(251,191,36,0.06)] border-accent-yellow/20',
  red: 'shadow-[0_0_24px_rgba(248,113,113,0.06)] border-accent-red/20',
  blue: 'shadow-[0_0_24px_rgba(96,165,250,0.06)] border-accent-blue/20',
}

export function Card({ title, subtitle, children, className = '', glow }: Props) {
  const glowClass = glow ? glowStyles[glow] : 'border-border-subtle'
  return (
    <div className={`bg-bg-card border rounded-2xl p-4 sm:p-5 ${glowClass} ${className}`}>
      {title && (
        <h3 className="text-[11px] uppercase tracking-[0.06em] text-text-muted font-semibold mb-1">
          {title}
        </h3>
      )}
      {subtitle && (
        <p className="text-[11px] leading-relaxed text-text-dim mb-3">{subtitle}</p>
      )}
      {!subtitle && title && <div className="mb-3" />}
      {children}
    </div>
  )
}
